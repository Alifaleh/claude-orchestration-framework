...
- Done = verified, never assumed: the gate ran and its output is shown, and the change was exercised with the real tools it ships with (tests, API calls, CLI runs). Test frequency ladder — per-edit: the slice via the test-gate hook (green is silent, zero agent turns) · per-beat: GATE_SCOPED by the engineer · per-wave: GATE_FULL + spec suites launched by the verifier in the background, off the critical path — the wave still closes only on green · per-milestone: fresh-install battery, full mutation run, full paid evals. Weight tiers mirror model routing: LIGHT (transcription — slice only) / STANDARD (scoped per beat + wave full) / TRIGGER-2 (full weight per beat, assertion-kill check included — never dieted). UI done = a headless Playwright SPEC passing for the changed behavior; an agent-driven browser drive only for flows with no spec yet — and that beat writes the spec; a UI-adjacent diff that changes no behavior needs neither.
